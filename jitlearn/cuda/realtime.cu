...
__constant__ Params params[{{ nr_projs  * nr_problems }}];
...
__global__ void cone_bp(
	...
	for (int p = 0; p < {{ nr_problems }}; ++p) {
        for (int i = 0; i < {{ nr_vxls_block_z }}; ++i)
            Z[i] = .0f;

		for (int j = start; j < end; ++j) {
			const float4& nU = params[p * {{ nr_projs }} + j].numeratorU;
			const float4& nV = params[p * {{ nr_projs }} + j].numeratorV;
			const float4& d  = params[p * {{ nr_projs }} + j].denominator;
        ...
        for (int i = 0; i < endZ; ++i)
            volumes[p][((startZ + i) * voxelsY + Y) * voxelsX + X]
                += Z[i] * outputScale;
	}
}