...
class ConvNd(nn.Module):
    ...
    def forward(self, input):
        ...
        if not self.is_transposed:
            ...
            if self.rank == 0:
                ...
                if self.padding_mode == 'zeros':
                    input = F.pad(input.view(input.shape[0], input.shape[1],
                                             input.shape[2], -1), padSize,
                                  'constant', 0).view(inputShape)
                else:
                    input = F.pad(input.view(input.shape[0], input.shape[1],
                                             input.shape[2], -1), padSize,
        ...
        if not self.is_transposed:
            ...
            size_ons = tuple(
                [size_i[x] - size_k[x] + 1 for x in range(len(size_i))])
        else:
            # Compute the size of the output tensor based on the zero padding
            size_o = tuple([(size_i[x] - 1) * self.stride[x] - 2 *
                            self.padding[x] + (size_k[x] - 1) + 1 +
                            self.output_padding[x] for x in
                            range(len(size_i))])

        # Output tensors for each 3D frame
        frame_results = size_o[0] * [
            torch.zeros((b, self.out_channels) + size_o[1:],
                        device=input.device)]
        ...
        for i in range(size_k[0]):
            ...
            for j in range(size_i[0]):
                ...
                if self.is_transposed:
                    ...
                    out_frame = j - (i - size_k[0] // 2) - (
                            size_i[0] - size_ons[0]) // 2 - (1 - size_k[0] % 2)
